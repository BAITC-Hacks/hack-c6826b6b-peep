...
В локальном демо вход разрешён только в эти заранее заведённые аккаунты. Пароли задаются при seed из `.env`; при отсутствии переменных `run.py` генерирует их и выводит один раз в локальную консоль. В БД хранится только хеш с солью. Не включать пароли в логи HTTP или репозиторий. Кнопки-пресеты на входе заполняют только логин.
...
LLM не ранжирует сотрудников, не выбирает действия вместо алгоритма, не меняет БД и не придумывает курсы. Числовые карточки всегда рендерятся из серверных полей. Свободный текст проходит ограничение длины и проверку упомянутых ID; если модель добавила непроверяемые численные утверждения, использовать шаблон целиком.

Таймаут 8 секунд, без повторного запроса в рамках одного действия. Ошибка, пустой ответ, неподходящий JSON или несовпадение claims → template fallback с `mode=template`. UI остаётся рабочим. На 30 минут кэшировать по сотруднику, типу вопроса, ревизии и версии промпта.

Провайдер в исходном роадмапе не выбран. Поэтому обязательная реализация — интерфейс, шаблонный провайдер и тестовый fake-провайдер; конкретный SDK подключается отдельно после выбора. Наличие реального LLM явно показывать в демо только при фактическом успешном вызове. Ключи — только в серверных переменных окружения.
...
| POST `/me/assistant/explain` | employee | question enum | `{text,mode,context_revision}` |
