...
Веб-админка доступна по `/admin`: `admin.demo` и `superadmin.demo`. Пароли задаются `DEMO_ADMIN_PASSWORD` и `DEMO_SUPERADMIN_PASSWORD` либо генерируются при первом запуске. Права проверяются сервером; HR видит только разрешённые разделы.

Управление сотрудниками, каталогом, сезонами, наградами, настройками AI и оформлением; предпросмотр публикации, аудит и корректировки начислений. Импорт данных и нативные сборки не входят в объём. Виртуальная выдача наград — тестовая.

Подробности: [руководство администратора](docs/11-admin.md), [проверки сборки](docs/build_matrix.md).
