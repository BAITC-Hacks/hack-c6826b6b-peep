...
def plan_context_for(s, eid, now, focus="balanced"):
    ...
    budget = max(0, prefs["weekly_minutes"] * 4 - saved["total_minutes"])
    rows = []
...
class OpenAIProvider:
    ...
    async def _request(self, body):
        async def call(client):
            response = await client.post("https://api.openai.com/v1/responses", json=body, headers={"Authorization": "Bearer " + os.getenv("OPENAI_API_KEY", "")}, timeout=15)
    ...
    async def explain(self, context):
        ...
        return await self._request({"model": os.getenv("OPENAI_MODEL", "gpt-4o-mini"), "store": False, "max_output_tokens": 900, "instructions": ASSISTANT_INSTRUCTIONS, "input": json.dumps({"task": "explain_career_context", "context": context}, ensure_ascii=False), "text": {"format": {"type": "json_schema", "name": "career_explanation", "strict": True, "schema": schema}}})

    async def compose_plan(self, context):
        ...
        return await self._request({"model": os.getenv("OPENAI_MODEL", "gpt-4o-mini"), "store": False, "max_output_tokens": 1100, "instructions": PLANNER_INSTRUCTIONS, "input": json.dumps({"task": "compose_verified_plan_draft", "context": context}, ensure_ascii=False), "text": {"format": {"type": "json_schema", "name": "career_plan_draft", "strict": True, "schema": schema}}})
...
class CareerAssistant:
    ...
    async def explain(self, context, eid, revision):
        ...
        if os.getenv("AI_MODE", "template") == "llm" and os.getenv("OPENAI_API_KEY"):
            try:
                candidate = await asyncio.wait_for(self.provider.explain(context), timeout=16)
    ...
    async def compose_plan(self, context, eid, revision):
        ...
        if os.getenv("AI_MODE", "template") == "llm" and os.getenv("OPENAI_API_KEY"):
            try:
                candidate = await asyncio.wait_for(self.provider.compose_plan(context), timeout=16)
                if verified_plan(candidate, context):
                    draft, mode = candidate, "llm"
            except (Exception, asyncio.TimeoutError):
                pass
        return self._put(key, _plan_result(draft, context, mode, revision))
