...
class Database:
    ...
    def ensure_accounts(self):
        with self.connection(write=True) as db:
            ...
            accounts = [('hr.demo', 'Hr123!', 'hr', None, 'HR-команда')]
            if ids:
                accounts.append(('employee.demo', 'Employee123!', 'employee', ids[0], 'Сотрудник'))
            if len(ids) > 1:
                accounts.append(('colleague.demo', 'Colleague123!', 'employee', ids[1], 'Коллега'))
            for username, password, role, employee_id, display_name in accounts:
