...
class SeasonProgressCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final maxLevel = _int(summary['max_level'] ?? 100);
    final level = _int(summary['level']).clamp(0, maxLevel);
    final start = _int(summary['level_start_xp'] ?? level * 200);
    final target = _int(summary['level_target_xp'] ?? (level + 1) * 200);
    final span = math.max(1, target - start);
    final xp = _int(summary['confirmed_xp']);
    final remaining = _int(summary['xp_to_next']);
    final maxed = level == maxLevel;
    final progress = maxed ? 1.0 : ((span - remaining) / span).clamp(0.0, 1.0);
    return Container(
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      maxed
                          ? 'Все $maxLevel уровней открыты. Забери свои награды.'
                    ...
                    Semantics(
                      ...
                      value: maxed ? '100%' : '${span - remaining} из $span XP',
                      child: ExcludeSemantics(
                        child: _XpBar(
                          progress: progress,
                          text: maxed
                              ? 'ПРОПУСК ПРОЙДЕН'
                              : '${span - remaining} / $span XP',
                        ),
                      ),
                    ),
                  ],
...
class _CareerPassRoadState extends State<CareerPassRoad> {
  ...
  void _onScroll() {
    ...
    final chapter = ((math.max(1, center) - 1) ~/ 10).clamp(
      0,
      math.max(0, (widget.levels.length - 1) ~/ 10),
    );
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 5),
                Text(
                  complete ? 'Все уровни позади. Время забрать заслуженное.' : 'Зарабатывай XP и открывай новые уровни. Каждый шаг — награда.',
                  style: const TextStyle(color: muted, fontSize: 13),
                ),
                const SizedBox(height: 14),
                Wrap(
            ...
            child: LayoutBuilder(
              builder: (context, constraints) {
                ...
                return Wrap(
                  ...
                  runSpacing: 6,
                  children: [
                    for (var i = 0; i < (widget.levels.length / 10).ceil(); i++)
                      SizedBox(
                        width:
                        ...
                        child: Tooltip(
                          message:
                              'Уровни ${i * 10 + 1}–${math.min((i + 1) * 10, widget.levels.length)}',
                          child: Semantics(
                            button: true,
                            ...
                            label:
                                'Перейти к уровням ${i * 10 + 1}–${math.min((i + 1) * 10, widget.levels.length)}',
                            child: Material(
                              color: _chapter == i
                              ...
                              child: InkWell(
                                ...
                                child: Container(
                                  ...
                                  child: Column(
                                    children: [
                                      Text(
                                        '${math.min((i + 1) * 10, widget.levels.length)}',
                                        style: TextStyle(
                                          fontSize: 12,
                                          fontWeight: FontWeight.w700,
